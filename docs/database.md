...
The live server (`server.py`) lets each lobby owner pick from the presets returned by `preset_registry.lobby_selectable_presets()` (see `docs/server.md`); `current` is the default. The `banned_cards.json` `domains` filter is applied to every preset — so if a procedure's pool overlaps the ban list heavily it may return fewer than the 15 domains required and the preset will fail to render.
